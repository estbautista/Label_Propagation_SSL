...
%
% Usage: [operator, estimation, classification] = standard_SSL_PR(A, mu, Y, varargin);
% Computes the standard SSL classification for the PageRank method
%
% INPUT: 
...
function [operator, estimation, classification] = standard_SSL_PR(A, mu, Y, varargin)
...
if nargin == 2
operator = ((1-alpha)*eye(N)) / (eye(N) - alpha*A*D^(-1));
end
