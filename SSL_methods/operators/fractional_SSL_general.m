...
% Tue Oct 24 21:19:02 CEST 2017
%
% Last edited on: 
% Tue Oct 24 21:19:15 CEST 2017
%
% Usage: [operator, estimation, classification] = fractional_SSL_general(A, sigma, mu, gamma, Y, varargin);
% Computes the generalized fractional SSL classification that has as particular cases the 
% Fractional PageRank, Fractional Normalized Laplacian and Fractional Standard Laplacian methods.
%
% INPUT: 
...
% ------ (sigma = 0) Fractional PageRank;
% ------ (sigma = 0.5) Fractional Normalized Laplacian;
% ------ (sigma = 1) Fractional Standard Laplacian;
...
function [operator, estimation, classification] = fractional_SSL_general(A, sigma, mu, gamma, Y, varargin)
...
if nargin == 4
operator = ((1-alpha)*eye(N)) / (eye(N) - alpha*Dg_sigma1*Ag*Dg_sigma2);
end
