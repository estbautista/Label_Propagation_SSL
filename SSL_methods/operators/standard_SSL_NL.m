...
%
% Usage: [operator, estimation, classification] = standard_SSL_NL(A, mu, Y, varargin);
% Computes the standard SSL classification for the Normalized Laplacian method
%
% INPUT: 
...
function [operator, estimation, classification] = standard_SSL_NL(A, mu, Y, varargin)
...
if nargin == 2
operator = ((1-alpha)*eye(N)) / (eye(N) - alpha*D^(-0.5)*A*D^(-0.5));
end
